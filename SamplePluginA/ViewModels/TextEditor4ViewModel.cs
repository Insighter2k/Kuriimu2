...
namespace SamplePluginA.ViewModels
{
    [Export(typeof(ITabItem))]
    [Export(typeof(Screen))]
    public sealed class TextEditor4ViewModel : Screen, ITabItem
    {
        [ImportingConstructor]
        protected TextEditor4ViewModel() { }

        [ImportingConstructor]
        protected TextEditor4ViewModel(string tabHeader, ImageSource icon, bool isSelected, KoreFile kFile)
        {
            TabHeader = tabHeader;
            TabIcon = icon;
        ...
        public static TextEditor4ViewModel Create(string tabHeader, ImageSource icon, bool isSelected, KoreFile kFile) => new TextEditor4ViewModel(tabHeader, icon, isSelected, kFile);
